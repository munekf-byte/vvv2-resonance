...
## 前任者履歴写真（Pro限定）容量監視
- バケット: `session-photos` (Free Plan / 1GB 上限)
- フル `~250KB` + サムネ `~30KB` ≒ 1 セッション最大 280KB
- 週 1 回 Dashboard → Storage → session-photos → Usage を確認
- **800MB 到達時にアラート** → 古い写真整理 or Pro Plan 移行を検討
- アップロードは `prev_photo_uploaded_at` を「両方成功時にのみ」更新するアトミック契約
...
| **統計分析レイヤースキル(横展開可)** | `.claude/skills/pachislot-analytics-layer/SKILL.md` |
| **収支入力UIスキル(横展開可)** | `.claude/skills/shushi-input-ui/SKILL.md` |
...
| 前任者履歴写真 セットアップ | `docs/photo-upload-setup.md` |
| **アクティベーションキー実装指示書 (note/BOOTH)** | `docs/activation-key-spec.md` |
