...
- **AT終了時（標準）**: モード再抽選。全確率をリセットし、設定別のデフォルト事前確率を適用。
- **AT終了時（駆け抜け）**: ATイベント行が **1行のみで終了** した場合（= 1戦目で対決勝利したがBITES/百足/梟に突入せず終了）→ 「通常A否定」を確定し、A=0 の状態で正規化したデフォルト事前確率を次ブロックに適用。
- **AT終了時（有馬ジャッジメント失敗）**: ATエントリ内に「有馬戦 result=失敗」行が含まれる場合 → 次ブロックを HEAVEN=1.0 で確定（100G以内CZ当選濃厚）。駆け抜け判定より優先。

---

### 1.3 AT駆け抜け / 有馬失敗 検出ロジック（v5.46〜）

#### 駆け抜けの定義
- 1つの AT エントリ（atKey 単位）内で生成された rows が **1行のみ** で終端する状態
- 構造的には「1戦目の対決に勝利したがバイツ獲得抽選で報酬が出ず、後続SETやBITES行が追加されなかった」ケース
- 実装上は `atEntry.rows.length === 1` で判定（rowType 種別は問わない）

#### 駆け抜けの恩恵
- **通常Aを完全否定** (`A = 0`)
- 残り 5モードはデフォルト事前確率を保持し、A 除外後に正規化

#### 有馬ジャッジメント失敗の定義
- AT エントリ内に `rowType === "arima"` かつ `result === "失敗"` の行が存在
- 該当する AT を獲得した通常時ブロックの **次のイベント行** が対象

#### 有馬失敗の恩恵
- **HEAVEN=1.0 ハード確定**（他モードすべて 0）
- 駆け抜け判定より優先（同時成立時は有馬失敗を採用）
